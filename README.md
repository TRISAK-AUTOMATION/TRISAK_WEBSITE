...
**Contact Requests** → `/admin/contact-requests` (sidebar: คำขอติดต่อ) — every submission from the public Contact Us form (`POST /api/contact`) is stored in the `contact_requests` table with status **New** and shown here newest-first. Admins can search by name / company / email / interested-in, filter by status, sort by submission date (click the *Submitted At* header), open a request to read the full message, change its status (**New → In Progress → Completed**), and delete it. All `/api/admin/contact-requests` endpoints require the admin session token. On an existing database, apply `migrations/020_create_contact_requests.sql` — it creates the table and copies over any rows from the old `contact_submissions` table (old statuses map: quoted/follow_up → In Progress, closed → Completed); the old table is left untouched. Fresh installs get the table from `schema.sql`. This replaces the earlier `/admin/leads` page (the old URL redirects).

**Sidebar → แก้ไข (Edit)** — an expandable/collapsible group in the Admin sidebar (click the label to toggle) with direct links to the content-managed pages: Home (above), History, Products, Contact Us, Automation Solution, Footer, and Pop-up. History / Products / Contact Us / Footer / Automation Solution (hero only) are bilingual editors (`/admin/about`, `/admin/products-page`, `/admin/contact`, `/admin/footer`, `/admin/automation-solution`) following the exact same real-tab-switch pattern as Home — backed by `about_content`, `products_page_content`, `contacts_page_content`, `footer_content`, and `automation_solution_page_content` (one singleton row each, `_en`/`_th` columns); apply `migrations/015_add_page_content.sql`, `migrations/016_add_footer_content.sql`, and `migrations/017_add_automation_solution_content.sql` on an existing database. The footer's head office/warehouse address, phone, and email are deliberately *not* duplicated into their own footer fields — they reuse `contacts_page_content`, so editing Contact Us updates both the Contact page and the footer at once. Scope note: for History, only the hero and intro text are DB-editable — the timeline, stats, org-structure, and partners sections stay static (unchanged) for now. There's no separate "Brands" item in this menu — brand name/logo/tagline/description is managed from the existing brand catalog (`/admin/brands`, under the "สินค้า" group below) rather than a page-content editor, since there's no separate "Brands" marketing page.

**Automation Solution → Solution Blocks** — the `/admin/automation-solution` editor also manages the page's "solution block" cards inline below the hero form: add, edit, and delete blocks, each with name, summary, services/benefits (one item per line), and an image (upload / replace / preview / delete via the same drag-and-drop field used elsewhere). This activates a `solutions` table + `GET /api/solutions` endpoint that already existed in the codebase but weren't wired to the frontend (the public page previously rendered its 3 blocks from static translation strings) — the public page now reads from this table, falling back to the original static text if the table is ever empty, so nothing changes visually until an admin edits or adds a block. Block name/summary/services/benefits are single-language (no EN/TH tab), matching how Brand/Category/Series names work elsewhere in this app; only the page's Hero text above is bilingual.
...
Each edit page (Brand/Category/Series/Product) has two tabs (**ข้อมูล** data / **อัพโหลด** upload-image), a sticky ยกเลิก/บันทึก (Cancel/Save) bar, and a right-hand sidebar panel for Language (display-only on these — Brand/Category/Series/Product names are still single-value, not per-language; the Home/About Us/Products/Contact Us/Footer editors under Sidebar → Edit are the ones that are genuinely bilingual), Brand/Category selects (Series only), and the Status toggle.
